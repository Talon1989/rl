...
#
#
# def val_iter(threshold=1e-16, gamma=0.99):
#     P = env.P
#     v = np.zeros(n_states)
#     while True:
#         old_v = v.copy()
...
#         for t, (s, a, _) in enumerate(episode):
#             if (s, a) not in s_a_pairs[:t]:
#                 total_reward[(s, a)] += np.sum(rewards[t:])
#                 n[(s, a)] += 1
#                 q[(s, a)] = total_reward[(s, a)] / n[(s, a)]
#     return pd.merge(
#         pd.DataFrame(q.items(), columns=['(s, a)', 'value']),
#         pd.DataFrame(n.items(), columns=['(s, a)', 'n']),
#         on='(s, a)'
...
def q_learning_2(alpha=0.3, gamma=0.99, n_iter=50_000):

    q = np.zeros([n_states, n_actions])

    def epsilon_greedy(s, e=0.3):
        if np.random.uniform(0, 1) < e or np.sum(np.abs(q[s])) == 0:
            return np.random.randint(0, n_actions)
        return np.argmax(q[s])

    epsilon, rate_e = 0.3, 0.0001
    for i in range(n_iter):
        s = env.reset()
        while True:
            if epsilon > 0.1:
    ...
    return np.array([np.argmax(q, axis=1)]), q


pol, s_a = q_learning_2()
